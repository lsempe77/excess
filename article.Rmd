...
- Enrique Acosta ^[Max Plank Institute for Demographich Research]
...
knitr::opts_chunk$set(echo = FALSE, warning = F,
  ...
  cache = T,fig.width = 7,fig.height = 5, cache.lazy = FALSE,
  digits = 1, OutDec = ",")
...
We estimate that registered mortality nationally underestimates mortality by `r pm`% (95% CI `r pml`% - `r pmu`%). We estimate total all-cause excess mortality during the period of analysis at `r et` (95% CI  `r etl` - `r etu`) of which `r er` (95% CI  `r erl` - `r eru`) were captured by the vital registration system.  Deaths at age 60 and over accounted for `r age60t`% (95% CI `r age60l`% - `r age60u`%) of total excess deaths, while there were fewer deaths than expected in younger age groups. Lima region, on the Pacific coast and including the national capital, accounts for `r limat` (95% CI  `r limal` - `r limau`) excess deaths, while the regions of Apurimac and Pasco, in the Andes, account for less than 400 excess deaths.
...
Then, we compute the completeness of deaths registration, $\widehat{\text{Completeness}}$ using the inverse logit of the predicted values from equation \@ref(eq:adair) and $\widehat{{\text{Excess}}_\text{Reg}}$ from equation \@ref(eq:rr), as follows:
...
Finally, we estimate total mortality during 2020 by adding $\widehat{\text{Excess}}_{\text{T}_{\text{min/mean/max}}}$ from equation \@ref(eq:finale) and the counterfactual difference of SINADEF deaths during 2020 and $\widehat{{\text{Excess}}_\text{Reg}}_{\text{min/mean/max}}$, adjusted by $\widehat{\text{Completeness}}$ as follows:
...
To adress the relative magnitude of mortality in 2020, we compute all cause age-standardised death rates per 1,000 people derived from the estimated total excess deaths. We use the direct standardization methods [@Curtin1995] based on population INEI population estimates by region and age-group for 2020 as the standard population [@INEI2009]. 
...
```{r map,fig.cap="Completeness of registration", warning=FALSE,message=FALSE}
...
ggplot() + geom_sf(data = DEPS,aes(fill=sub.mean),size=.01,colour="white")+ 
  ...
  scale_fill_viridis_c(name = "Completeness of registration (%)",
                       direction=-1)+
  geom_sf_text(data=DEPS,aes(label = Departamento),size=2)+
...
Table \@ref(tab:exineidepq) shows estimates by region. Lima, which includes the capital, accounts for  `r lima.total` total excess deaths, and `r menos5` show the lowest numbers. Table \@ref(tab:exineiageq) shows excess mortality estimates by age group. Deaths among people aged 60 years and over accounts for `r pr1`% of total excess mortality. There was negative excess registered mortality for the two youngest age groups: under 9 and 10 to 19.
...
pander(exineiageq[1:8], caption = "(\\#tab:exineiageq) Estimated total excess deaths by age-group", split.table = Inf)

```

Our second term, excess unregistered deaths, is estimated to be `r ex.unreg`. This represents `r ex.unreg.pc` of our estimate of the total registered and unregistered deaths. Coastal regions show the highest values where Lima accounts for `r lima.more` additional deaths, followed by Piura (`r piura.more`) and Lambayeque (`r lamba.more`). Our third term adds `r complete.covid` deaths corresponding to cases when reported COVID-19 deaths exceed our estimate of adjusted excess registered mortality. This mainly occurs for some smaller groups at younger ages. The regions with the highest under reported COVID-19 values are the Andean regions of Ayacucho (`r ayacucho.covid`), Cajamarca and Puno (both with `r puno.covid`).

Combining all these terms, our estimate of total excess deaths during 2020 is `r e.total` and our estimate of total deaths for 2020 is `r total`. This is `r excess.pct.t` more than the number of deaths projected by INEI for 2020. 

Figure \@ref(fig:map2) shows all cause age-standardised death rates.  We find that `r adj.rates.mores` (CI 95% ranging from `r adj.rates.ci.min` to `r adj.rates.ci.max`) present the highest rates, ranging from `r min` to `r max`, while regions of `r adj.rates.less` show the lowest rates ranging from `r min2` to `r max2` (CI 95% ranging from `r adj.rates.ci.l.min` to `r adj.rates.ci.l.max`) (left). We find that `r dif` shows `r dif.n` deaths per 1000 in comparison to INEI's projections for 2020, which suggests that INEI overestimates mortality rates in Amazonas (right.

<!-- ***This affects our estimation of completeness.*** -->

``` {r map2, fig.cap="Age-standardised death rates"}
...
mr1<-ggplot() + geom_sf(data = DEPS,
  ...
  geom_sf_text(data=DEPS,aes(label = Departamento),size=2)+
  theme_map() + ggtitle("Estimation of Age-standardised death rates in 2020")+ 
...
mr2<-ggplot() + geom_sf(data = DEPS,
  ...
  scale_fill_viridis_c( direction=1,option = "plasma",
                        name = "Difference per 1,000 deaths")+
  geom_sf_text(data=DEPS,aes(label = Departamento),size=2)+
  theme_map()+ ggtitle("Difference between INEI projections and current estimation")+ 
  theme(panel.background = element_rect(fill = 'white'),legend.position="bottom")

ggarrange(mr1,mr2,ncol = 2)
...
sub.reg.t<-sub.reg %>%dplyr::select(Departamento,sub.mean) %>%
  mutate(sub.mean=sub.mean*100) %>% arrange(-sub.mean)

pander(sub.reg.t, caption = "(\\#tab:under) Under-registration rates",
       split.table=Inf)
...
pander(mortality.rates, caption = "(\\#tab:standard) Age-standardised deaths rates",
       split.table=Inf)
...
pander(smrd.q, caption = "(\\#tab:count) Counterfacutual and total deaths 2020",
       split.table=Inf)
...
pander(propreg, caption = "(\\#tab:count) Proportion total excess deaths per region",
       split.table=Inf)
...
pander(popu, caption = "(\\#tab:count) Population 2020 per region",
       split.table=Inf)
