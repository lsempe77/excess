...
``` {r map2, fig.cap="Age-standardised mortality rates"}

mr1<-ggplot() + geom_sf(data = DEPS,
                       aes(fill=adj.rate),size=.01,colour="white") +
  scale_fill_viridis_c( direction=1,option = "plasma",
                        name = "Age-standardised mortality rate per 1,000")+
  geom_sf_text(data=DEPS,aes(label = Departamento),size=2)+
  theme_map() +
...
mr2<-ggplot() + geom_sf(data = DEPS,
  ...
  scale_fill_viridis_c( direction=1,option = "plasma",
                        name = "Difference between INEI's projections and 2020 estimates")+
  geom_sf_text(data=DEPS,aes(label = Departamento),size=2)+
  theme_map()+
  theme(panel.background = element_rect(fill = 'white'),legend.position="bottom",
        legend.title=element_text(size=7))
