...
colnames(pop.reg.ed)[2]<-"range"


SINADEF.causal4<-SINADEF %>%
  mutate(FECHA = as.Date(FECHA, "%Y-%m-%d")) %>%
  filter(FECHA >= "2018-01-01")%>%
  group_by(Departamento,FECHA,range)%>%
  summarise(deaths=n())%>% dplyr::select(deaths,FECHA,range,Departamento) %>%
  mutate(covid = case_when(FECHA > "2020-03-15" ~ 1,
                           T ~ 0)) %>% filter(!is.na(range))

SINADEF.causal4 <- SINADEF.causal4 %>% left_join(pop.reg.ed)


its.covid.list4b<-SINADEF.causal4 %>% group_by(Departamento,range) %>%
...
its.covid.list4d<-SINADEF.causal4 %>% group_by(Departamento,range) %>%
...
its.covid.list4d.l<-SINADEF.causal4 %>% group_by(Departamento,range) %>%
...
its.covid.list4b.q<-SINADEF.causal4 %>% group_by(Departamento,range) %>%
  do(its.covid = glance(glm(deaths ~ splines::bs(FECHA,knots=40) + covid+
                              factor(wday(FECHA))+
                              offset(log(population)),
...
its.covid.list4d.q<-SINADEF.causal4 %>% group_by(Departamento,range) %>%
...
its.covid.list4d.l.q<-SINADEF.causal4 %>% group_by(Departamento,range) %>%
  filter(Departamento=="LAMBAYEQUE") %>%
  filter(FECHA > 2018-01-01) %>%
  do(its.covid = tidy(coeftest(glm(deaths ~ splines::bs(FECHA,knots=40) + covid+
                                     offset(log(population))+factor(wday(FECHA)),
...
nbfit<-SINADEF.causal4 %>% group_by(Departamento,range) %>%
...
nbfit2<-SINADEF.causal4 %>% group_by(Departamento,range) %>%
